...
########## BS_vS_EM_correlation ########## 
p_BS <- data.table(chr=BS$chr,pos=BS$pos,cover=BS$mc+BS$nmc,ratio=BS$mc/(BS$mc+BS$nmc)) %>%
  filter(cover>=3) %>% mutate(chrpos=paste0(chr,"_",pos))
p_EM <- data.table(chr=EM$chr,pos=EM$pos,cover=EM$mc+EM$nmc,ratio=EM$mc/(EM$mc+EM$nmc)) %>%
  filter(cover>=3) %>% mutate(chrpos=paste0(chr,"_",pos))
df <- merge(p_BS,p_EM,by="chrpos")
cor(df$ratio.x,df$ratio.y,method="pearson")

p_BS <- data.table(chr=BS$chr,pos=BS$pos,cover=BS$mc+BS$nmc,ratio=BS$mc/(BS$mc+BS$nmc)) %>%
  filter(chr=="Chr4") %>% filter(cover>=50) 
p_EM <- data.table(chr=EM$chr,pos=EM$pos,cover=EM$mc+EM$nmc,ratio=EM$mc/(EM$mc+EM$nmc)) %>%
  filter(chr=="Chr4") %>% filter(cover>=50) 
df <- merge(p_BS,p_EM,by="pos")
rm(p_BS,p_EM)
p <- sample_n(df,10000)
ggplot(p,aes(x=ratio.x,y=ratio.y)) +
  geom_point(alpha=0.3) +
  xlab("BS methylation ratio") +
  ylab("EM methylation ratio") + 
  theme(panel.background=element_blank(),
        panel.border=element_blank(), 
        panel.grid.major=element_blank(),
        panel.grid.minor=element_blank(), 
